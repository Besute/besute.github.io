...
function GetReady() {
  ...
  return ( !data ? 
    <div className="hello__div" onMouseMove={onMouseMove}>
      ...
      <button className="hello__button" onClick={() => {setData(!data)}}>Продолжить...</button>
    ...
    : !ready ? 
      <div className="hello__div">
        <h1 className="hello__hello hello__hello_4">Оказывается, ты любишь ходить в театры</h1>
        <h2 className="hello__hello hello__hello_5">НАСКОЛЬКО сильное У ТЕБЯ желание туда пойти?</h2>
        <div className="button__div">
          <button className="hello__button_2" onClick={() => {setReady(!ready)}}>ОГРОМНОЕ</button>
          <button className="hello__button_3" onClick={() => {
            document.querySelector(".hello__button_3").style.top = `${((Math.random() * Math.random() * 1007) % 100 + 100) % 100}%`
            document.querySelector(".hello__button_3").style.left = `${((Math.random() * Math.random() * 1007) % 100 + 100) % 100}%`
            document.querySelector(".hello__button_3").style.position = "absolute"
          }}>Да его почти и нет...</button>
        </div>
      </div>
    : !data2 ?
    <div className="hello__div">
